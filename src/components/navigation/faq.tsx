            ...
            <Tab.Panels className="mt-5">
                {Object.values(categories).map((info, idx) => (
                    <Tab.Panel key={idx} className="flex-box space-y-5 font-minecraft text-white md:text-3xl"> 
                        {info.map((section) => (
                          <div className={section.open ? 'bg-[#8e8e8e] h-100% shadow-[inset_-5px_-5px_0px_rgb(113,113,113),inset_5px_5px_0px_rgb(177,177,177)]':'mBG'}>
                            <Disclosure >
                              <Disclosure.Button onPointerUp={() => {section.open = true}} className={({ open }) =>
                                  classNames(
                                ...
                                <span >{section.title}{section.open}</span>
                              </Disclosure.Button>
