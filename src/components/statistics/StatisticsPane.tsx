          ...
          >
            {statistic.name!=="discord"
              ? <div className="flex justify-center items-center flex-col">
                  <h1 className="text-3xl sm:text-6xl lg:text-8xl duration-500">{statistic.statistic}</h1>
                  <br className="hidden sm:block" />
                  <h4 className="text-sm sm:text-2xl lg:text-3xl uppercase duration-500">{statistic.name}</h4>
              </div> : <>
                {Object.entries(socialIcons).map(([key, value]) => (
                  <SocialIcon key={key} title={key} logo={value.img} url={value.url} dimensions="w-8 sm:w-12 lg:w-16 duration-500" />
                ))}
            </>}
          </div>;
        })}
